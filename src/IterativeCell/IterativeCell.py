...
class IterativeCell(tf.nn.rnn_cell.RNNCell):
    ...
    def resolve_iteration_calculation(self, input, state, number_of_iterations_performed, scope):
        ...
        new_state_activation = array_ops.concat(1, [self._iteration_activations, self._iteration_activations])
        new_state = new_state * new_state_activation + state * (1 - new_state_activation)
