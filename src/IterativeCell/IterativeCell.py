...
class IterativeCell(tf.nn.rnn_cell.RNNCell):
    def __init__(self, internal_nn, iteration_activation_nn=None, max_iterations=10., initial_iterate_prob=0.75, iterate_prob_decay=0.75, allow_cell_reactivation=True, add_summaries=False):
    ...
    def __call__(self, input, state, scope=None):
        ...
        if self._should_add_summaries:
            self.add_post_execution_summaries(final_output, final_state, number_of_iterations_performed, final_iterate_prob, final_iteration_activations)
    ...
    def resolve_iteration_activations(self, input, old_state, output, new_state, iterate_prob, current_iteration_activations):
        if self._iteration_activation_nn is not None:
            ...
            iteration_activations = sigmoid(linear([input, new_state], self.output_size, True, scope=tf.get_variable_scope()))
        tf.get_variable_scope().reuse_variables()

        discrete_iteration_activations = tf.floor(iteration_activations + iterate_prob)
        return self.update_iteration_activations(current_iteration_activations, discrete_iteration_activations, iterate_prob)
